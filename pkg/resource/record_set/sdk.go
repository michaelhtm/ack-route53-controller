...
func (rm *resourceManager) sdkFind(
	...
	for _, elem := range resp.ResourceRecordSets {
		...
		if elem.ResourceRecords != nil {
			f9 := []*svcapitypes.ResourceRecord{}
			for _, f9iter := range elem.ResourceRecords {
				f9elem := &svcapitypes.ResourceRecord{}
				if f9iter.Value != nil {
					f9elem.Value = f9iter.Value
				}
				f9 = append(f9, f9elem)
			}
			ko.Spec.ResourceRecords = f9
		} else {
		...
		if r.ko.Spec.ChangeBatch.Changes != nil {
			...
			for _, f0f0iter := range r.ko.Spec.ChangeBatch.Changes {
				...
				if f0f0iter.ResourceRecordSet != nil {
					...
					if f0f0iter.ResourceRecordSet.GeoProximityLocation != nil {
						f0f0elemf1f4 := &svcsdktypes.GeoProximityLocation{}
						if f0f0iter.ResourceRecordSet.GeoProximityLocation.AWSRegion != nil {
							f0f0elemf1f4.AWSRegion = f0f0iter.ResourceRecordSet.GeoProximityLocation.AWSRegion
						}
						if f0f0iter.ResourceRecordSet.GeoProximityLocation.Bias != nil {
							if *f0f0iter.ResourceRecordSet.GeoProximityLocation.Bias > math.MaxInt32 || *f0f0iter.ResourceRecordSet.GeoProximityLocation.Bias < math.MinInt32 {
								return nil, fmt.Errorf("error: field Bias is of type int32")
							}
							biasCopy := int32(*f0f0iter.ResourceRecordSet.GeoProximityLocation.Bias)
							f0f0elemf1f4.Bias = &biasCopy
						}
						if f0f0iter.ResourceRecordSet.GeoProximityLocation.Coordinates != nil {
							f0f0elemf1f4f2 := &svcsdktypes.Coordinates{}
							if f0f0iter.ResourceRecordSet.GeoProximityLocation.Coordinates.Latitude != nil {
								f0f0elemf1f4f2.Latitude = f0f0iter.ResourceRecordSet.GeoProximityLocation.Coordinates.Latitude
							}
							if f0f0iter.ResourceRecordSet.GeoProximityLocation.Coordinates.Longitude != nil {
								f0f0elemf1f4f2.Longitude = f0f0iter.ResourceRecordSet.GeoProximityLocation.Coordinates.Longitude
							}
							f0f0elemf1f4.Coordinates = f0f0elemf1f4f2
						}
						if f0f0iter.ResourceRecordSet.GeoProximityLocation.LocalZoneGroup != nil {
							f0f0elemf1f4.LocalZoneGroup = f0f0iter.ResourceRecordSet.GeoProximityLocation.LocalZoneGroup
						}
						f0f0elemf1.GeoProximityLocation = f0f0elemf1f4
					}
					...
					if f0f0iter.ResourceRecordSet.ResourceRecords != nil {
						f0f0elemf1f9 := []svcsdktypes.ResourceRecord{}
						for _, f0f0elemf1f9iter := range f0f0iter.ResourceRecordSet.ResourceRecords {
							f0f0elemf1f9elem := &svcsdktypes.ResourceRecord{}
							if f0f0elemf1f9iter.Value != nil {
								f0f0elemf1f9elem.Value = f0f0elemf1f9iter.Value
							}
							f0f0elemf1f9 = append(f0f0elemf1f9, *f0f0elemf1f9elem)
						}
						f0f0elemf1.ResourceRecords = f0f0elemf1f9
					}
...
func (rm *resourceManager) terminalAWSError(err error) bool {
	...
	switch err.(type) {
	case *svcsdktypes.InvalidChangeBatch,
		*svcsdktypes.InvalidInput,
		*svcsdktypes.NoSuchHostedZone,
		*svcsdktypes.NoSuchHealthCheck:
		return true
	default:
		return false
	}
